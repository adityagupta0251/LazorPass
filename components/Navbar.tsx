...
const Navbar: React.FC = () => {
  ...
  const [openDropdown, setOpenDropdown] = useState<"left" | "center" | "wallet" | null>(null);
  ...
  const [solPrice, setSolPrice] = useState(98.50); 
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-40 transition-all duration-200 ${
        isScrolled ? "backdrop-blur-md bg-black/75" : "backdrop-blur-sm bg-black/40"
      ...
      <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4 sm:h-16 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <div className="relative" ref={walletDropdownRef}>
            ...
            >
              <span className={`h-1.5 w-1.5 rounded-full animate-pulse ${isConnected ? "bg-emerald-400" : "bg-slate-400"}`} />
              <span>{isConnecting ? "Connecting..." : isConnected ? displayAddress : "Connect Wallet"}</span>
              {isConnected && (
                <span className={`transition-transform ${openDropdown === "wallet" ? "rotate-180" : ""}`}>▾</span>
              )}
            </button>
                    ...
                    <>
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <div className="text-xs text-slate-500">
                            {solBalance !== null ? `≈ $${(solBalance * solPrice).toFixed(2)}` : '...'}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="text-xs text-slate-400">USD Coin</div>
                            <div className="text-sm font-semibold text-white">
                        ...
                        <div className="text-right">
                          <div className="text-xs text-slate-500">
                            {usdcBalance !== null ? `≈ $${formatBalance(usdcBalance, 2)}` : '...'}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="pt-2 mt-2 border-t border-white/5">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-sm font-bold text-emerald-400">
                            {totalBalanceUSD !== null ? `$${totalBalanceUSD.toFixed(2)}` : '...'}
                          </span>
                        </div>
                      </div>
                      ...
                      >
                        <svg className={`w-3 h-3 ${balancesLoading ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          >
            <div className="relative h-4 w-4">
              <span className={`absolute left-0 top-0 h-[1.5px] w-full bg-slate-100 transition-all ${isMobileMenuOpen ? "translate-y-1.5 rotate-45" : ""}`} />
              <span className={`absolute left-0 top-1.5 h-[1.5px] w-full bg-slate-100 transition-all ${isMobileMenuOpen ? "opacity-0" : ""}`} />
              <span className={`absolute left-0 top-3 h-[1.5px] w-full bg-slate-100 transition-all ${isMobileMenuOpen ? "-translate-y-1.5 -rotate-45" : ""}`} />
            </div>
          </button>
        </div>
      </div>
          ...
          <div className="rounded-2xl border border-slate-700/70 bg-black/95 p-4 space-y-4 shadow-2xl">
            ...
            {isConnected && (
              <div className="bg-gradient-to-br from-emerald-500/10 to-transparent rounded-xl p-3 border border-emerald-500/20">
                ...
                <div className="font-mono text-xs text-slate-300 mb-3">{displayAddress}</div>
                  ...
                  <>
                    <div className="space-y-2 mb-3">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="text-white font-semibold">{formatBalance(solBalance)}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-slate-400">USDC:</span>
                        <span className="text-white font-semibold">{formatBalance(usdcBalance, 2)}</span>
                      </div>
                    ...
                    <div className="flex gap-2 mb-2">
                      <button onClick={copyAddress} className="flex-1 py-2 px-3 rounded-lg bg-white/5 border border-white/10 text-xs font-medium text-white">
                        Copy
                      </button>
                      <button onClick={viewOnExplorer} className="flex-1 py-2 px-3 rounded-lg bg-white/5 border border-white/10 text-xs font-medium text-white">
                        Explorer
                      </button>
                    </div>
            ...
            <div className="pt-2 border-t border-white/5">
              <button
                onClick={() => isConnected ? handleDisconnect() : router.push("/SolanaPayWidget")}
